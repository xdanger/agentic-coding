...
- `/utils/doc_linter.py`：文档标准检查工具，可以检查文件名、目录名、文件引用格式、代码块格式等是否符合规范。
...
class DocumentationLinter:
    ...
    def check_file_naming(self, file_path):
        ...
        if " " in name_without_extension:
            self.errors.append(
                f"File name contains spaces, use hyphens instead: {file_path}"
            )
        ...
        if "_" in name_without_extension and not filename.startswith("."):
            ...
            if not exempt_from_underscore:
                self.errors.append(
                    f"File name contains underscores, use hyphens instead: {file_path}"
                )
    ...
    def check_directory_naming(self, file_path):
        ...
        for part in parts:
            ...
            if not part.islower():
                self.errors.append(
                    f"Directory name should be lowercase: {part} in {dir_path}"
                )

            if " " in part:
                self.errors.append(
                    f"Directory name contains spaces, use hyphens instead: {part} in {dir_path}"
                )

            if "_" in part and not part.startswith("."):
                self.errors.append(
                    f"Directory name contains underscores, use hyphens instead: {part} in {dir_path}"
                )
            ...
            for collection in collection_dirs:
                if part == collection[:-1]:  # Singular form of a collection
                    self.warnings.append(
                        f"Collection directory should use plural form: {part} should be {collection} in {dir_path}"
                    )
    ...
    def check_file_references(self, file_path):
        ...
        try:
            ...
            try:
                ...
                self.warnings.append(
                    f"Could not read file {file_path} due to encoding issues: {str(e)}"
                )
                return
        ...
        for line in lines:
            ...
            for common_dir in common_dirs:
                ...
                for match in re.finditer(pattern, line):
                    self.warnings.append(
                        f"Directory reference should use backticks: {match.group(1)} at {file_path}:{line_number}"
                    )
            ...
            for match in re.finditer(extension_pattern, line):
                ...
                context_before = (
                    line[: match.start()].strip().split()[-1]
                    if line[: match.start()].strip()
                    else ""
                )
                if not context_before.endswith(("http:", "https:", "www.", "ftp:")):
                    self.warnings.append(
                        f"File extension reference should use backticks: {match.group(1)} at {file_path}:{line_number}"
                    )
        ...
        for i, line in enumerate(lines):
            ...
            if line.strip().startswith("```"):
                if not in_code_block:  # Start of code block
                    ...
                    if line.strip() == "```":
                        self.errors.append(
                            f"Code block doesn't specify a language, use ```plaintext for text: {file_path}:{i + 1}"
                        )
                    else:
        ...
        for line in lines:
            ...
            if line.strip().startswith("#"):
                ...
                if not heading_levels:
                    ...
                    if level != 1:
                        self.warnings.append(
                            f"Document should start with a level 1 heading (# Title): {file_path}:{line_number}"
                        )
                else:
                    ...
                    if level > heading_levels[-1] + 1 and not (
                        level == 3 and heading_levels[-1] == 1 and "." in line
                    ):
                        self.errors.append(
                            f"Heading level skipped (from {heading_levels[-1]} to {level}): {file_path}:{line_number}"
                        )
            ...
            if re.match(r"^\s*\*\s", line):
                self.warnings.append(
                    f"Use - for list items instead of *: {file_path}:{line_number}"
                )
    ...
    def report(self):
        ...
        report.append(
            f"Found {len(self.errors)} errors and {len(self.warnings)} warnings."
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Documentation Linter for Agentic Coding Framework"
    )
    parser.add_argument(
        "--root", default=os.getcwd(), help="Root directory of the repository"
    )
